...
SELECT  post_id,poster_id,posterName,bandName,date
FROM

    (SELECT user_follower.followingUserId as follower, post.private as private, post.id as post_id,content.creatorId as poster_id, mb_user.name as posterName, 'na' as bandName, content.date as date--, content.text as postText
    FROM post
    ...
    SELECT band_follower.userId as follower, post.private as private, post.id as post_id,content.creatorId as poster_id,mb_user.name as posterName,band.name as bandName,  content.date as date--, content.text as postText
    FROM post
    JOIN content ON content.id = post.contentId
    JOIN mb_user ON mb_user.id = content.creatorId
    JOIN band_follower ON band_follower.bandId = post.bandId
    JOIN band ON band.id = band_follower.bandId) as all_posts
